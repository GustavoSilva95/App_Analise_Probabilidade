...
def preco_por_desvio():
    ...
    for i, d in enumerate(desvios):
        global pr
        pr = preco_atual * math.exp((tx_juro - vol**2/2) * prazo/dias_ano + (d) * vol * math.pow(prazo / dias_ano, 1/2))
        if d == 1 or d == -1:
...
def probabilidade():
    d1_abaixo = (np.log(preco_atual / menor_preco) + (tx_juro - 1/2 * vol**2) * prazo / dias_ano) / (vol * math.pow(2 * prazo / dias_ano, 1/2))

    d1_acima = (np.log(maior_preco / preco_atual) - (tx_juro - 1/2 * vol**2) * prazo / dias_ano) / (vol * math.pow(2 * prazo / dias_ano, 1/2))

    erfc_d1_abaixo = 2 - 2 * norm.cdf(d1_abaixo * math.pow(2, 1/2))
    erfc_d1_acima = 2 - 2 * norm.cdf(d1_acima * math.pow(2, 1/2))
